...
def inv_sampling_Rouse(a,Ca,Hmax,beta,nsample):
    ...
    sample = np.linspace(loc, loc + scale, num=nsample)
    
    Zinv = f(sample) # random coordinates of plastic samples flyig in suspension following the Rouse(_,a,Ca,Hmax,beta) profile.
    return Zinv
...
def visz_data(data):
    ...
    # Hydro
    kappa = 0.41
    z = z + WaterLev # From here on, zero at the free surface (i.e.: zp = z-h)
    ...
    plt.hist(z, bins=20, orientation="horizontal")
...
def get_Ca(z, a, Hmax, beta, Csus):
    ...
    diff = 100*np.ones(len(Ca_list))
    
    for i in range(0,len(Ca_list)):
        C = Rouse(z, a, Ca_list[i], Hmax, beta)
        
        """
        The next step: we count how many plastics are in the entire water 
        column and substract to the ones counted in the experiment.
        In other words: we have the same total number of samples in our Rouse
        and in the experiment (or as close as possible for a Ca which is an
        integer)
        """
        diff[i] = Csus-np.nansum(C)

    
    # we can calculate this better:
    diff_zero_i = np.argmin(diff**2)
    # print("Different Ca tested: \n", Ca_list)
    # print("Error for different Ca: \n", diff**2)
    # print("Position of the minimum error:: \n", diff_zero_i)    
    # print("Ca of min error: \n",  Ca_list[diff_zero_i])

    Ca = Ca_list[diff_zero_i]
